...
//マーカーResource
#[derive( Resource )] pub struct AfterNowLoading ( pub MyState );
...
{   let mut rng = rand::thread_rng();
    ...
    let custom_size = Some ( Px2d::new( PIXELS_PER_GRID, PIXELS_PER_GRID ) * 0.75 );
...
(   assets   : Res<LoadedAssets>,
    ...
    o_marker : Option<Res< AfterNowLoading >>,
    asset_svr: Res<AssetServer>,
)
{   //プリロードが完了したか？
...
{   let time_delta = time.delta().as_secs_f32() * 5.0;
    ...
    (   | ( mut transform, goal_xy ) |
        {   let mut goal = goal_xy.0.to_screen_pixel();
