...
{	pub fn find( & self, player: &Player, maze: &GameMap, cmds: &mut Commands, ) -> Option<(DxDy,DxDy)>
	...
	{	let mut x1 = self.grid.x;
		...
		{	//追手の移動方向の候補（壁の有無は考慮してない）
			...
			let dx = PIXELS_PER_GRID * side_x as f32 / side_y as f32;
			let dy = PIXELS_PER_GRID * if y1 >= y2 { 1.0 } else { -1.0 };
			...
			{	let mut n = 0;
				loop
				{	let new_px = pixel.x + PIXELS_PER_GRID * n as f32;
					if new_px >= pixel.x + dx + adjust { break } //内側loopの脱出条件

					//壁か？
					grid.x = ( ( new_px - ( PIXELS_PER_GRID - SCREEN_WIDTH ) / 2.0 ) / PIXELS_PER_GRID ) as i32;
				...
				pixel += ( dx + adjust, dy );
				let work = ( pixel.x / PIXELS_PER_GRID ).floor() * PIXELS_PER_GRID;
				pool += pixel.x - work;
				pixel.x = work;			//次の開始位置
				adjust = ( pool / PIXELS_PER_GRID ).floor() * PIXELS_PER_GRID;
				pool -= adjust;
			}
		}
		else
		{	//追手の移動方向の候補（壁の有無は考慮してない）
			...
			//pixelの増加量を求める（短辺は＋１または－１ずつ進める）
			let dx = PIXELS_PER_GRID * if x1 <= x2 { 1.0 } else { -1.0 };
			let dy = PIXELS_PER_GRID * side_y as f32 / side_x as f32;
			...
			{	let mut n = 0;
				loop
				{	let new_py = pixel.y - PIXELS_PER_GRID * n as f32;
					if new_py <= pixel.y - dy - adjust { break } //内側loopの脱出条件

					//壁か？
					grid.y = ( ( ( SCREEN_HEIGHT - PIXELS_PER_GRID ) / 2.0 - new_py ) / PIXELS_PER_GRID ) as i32 - 1;
					if maze.is_wall( grid ) { return None }	//関数の脱出条件
					n += 1;
				...
				pixel += ( dx, - ( dy + adjust ) );
				let work = ( pixel.y / PIXELS_PER_GRID ).ceil() * PIXELS_PER_GRID;
				pool += work - pixel.y;
				pixel.y = work;			//次の開始位置
				adjust = ( pool / PIXELS_PER_GRID ).floor() * PIXELS_PER_GRID;
				pool -= adjust;
